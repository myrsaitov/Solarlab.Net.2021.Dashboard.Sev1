...
namespace Sev1.UserFiles.Application.Validators.UserFile
{
    ...
    /// </summary>
    public class UserFilesUpdateDtoValidator : NullReferenceAbstractValidator<UserFileUpdateDto>
    {
        public UserFilesUpdateDtoValidator()
        {
            // Общая проверка
            RuleFor(x => x)
                .NotNull()
                .NotEmpty().WithMessage("UserFileUpdateDto is null!");
