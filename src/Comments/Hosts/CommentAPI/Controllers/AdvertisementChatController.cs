...
{
    /// <summary>
    /// API для комментариев к объявлению  SellerConsumerChatController
    /// </summary>
    [Route("api/[controller]")]
    [ApiController]
    [ServiceFilter(typeof(CommentsExceptionFilter))]
    public class AdvertisementChatController : ControllerBase
    {
        private readonly ICommentsService _commentService;

        /// <param name="commentService"></param>
        public AdvertisementChatController(ICommentsService commentService)
        {
            _commentService = commentService;
        }

        /// <summary>
        /// Получить коментарии объявления, paged
        /// </summary>
        /// <response code="200">Ok</response>
        [HttpGet("GetCommentsByAdvertisementIdPaged")]
        public async Task<IActionResult> GetCommentsByAdvertisementIdPaged([FromQuery] CommentDtoRequestGetByChatId dto, CancellationToken token = default)
        {
        ...
        public async Task<IActionResult> DeleteChatByAdvertisementId([FromQuery] Guid id, CancellationToken token = default)
        {
            if (ModelState.IsValid)
            {
